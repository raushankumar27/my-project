...
testcollage__c = function (opts) {
	...
	function createhtmlbody() {
		...
		htmlstr += "				<input type=file  accept=\"image\/*\" id=localinput multiple>";
	...
	function oninit() {
		try {
			...
			createhtmlbody();
			//loadCSSFile("http://f5_friday.os.a2zapps.com/api/v4/core/common/page/collagemakercss__c");
	...
	function imagesaver(type) {
		//remove unwanted item
		showselrect(false);
		var svgData = $("#" + PAGE_OPTIONS.elemId).find("svg")[0].outerHTML;
		var svgBlob = new Blob([svgData], { type: "image/svg+xml;charset=utf-8" });
		var svgUrl = URL.createObjectURL(svgBlob);
		var image = new Image;
		image.onload = function (e) {
